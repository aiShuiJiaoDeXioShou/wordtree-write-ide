...
public class FileItemComponent extends TreeItem<Label> {
        ...
        this.expandedProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue) {
                ...
                for (File f : Arrays.stream(file.listFiles()).sorted((o1, o2) -> {
                    ...
                    var menu = new FileItemComponent(f);
    ...
    private void newFileOrFolder(TreeItem<Label> fileTree, FileService fileService, @NotNull Label target, ActionEvent e, int type) {
        ...
        var alert = new WTFxInputAlert("新建", alert1 -> {
            ...
            if (b) {
                var treeItem = new FileItemComponent(new File(idStr));
    ...
    private void addTab() {
        ...
        BeanFactory.nowWorkSpace.set(build);
        if (build instanceof WriterEditor code) {
            addCode(tab, code);
    ...
    private void addCode(Tab tab, CodeArea code) {
        ...
        if (!(code instanceof LangEditor)) code.appendText(context);
            ...
            ThreadUtil.execAsync(() -> {
                ...
                if (bool) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        } else SystemMessage.sendError("错误: 这是根节点您不能删除!");
                    });
                } else {
                    SystemMessage.sendWarning("文件删除失败");
                }
            });
        });
    }
